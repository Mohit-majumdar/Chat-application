...
app = Flask(__name__)
app.config["SECRET_KEY"] = "khj khksaddsfajhdfshakfjhakjshfjhhjk"
socketio = SocketIO(app)
rooms = {}


def genrate_room_code():
    while True:
        code = ""
        for _ in range(5):
...
def home():
    ...
    if request.method == "POST":
        ...
        if join != False and not code:
            return render_template(
                "home.html", error="Please Enter room code", code=code, name=name
            )

        room = code

        if create != False:
            room = genrate_room_code()
            rooms[room] = {"member": 0, "messages": []}

        elif code not in rooms:
            return render_template(
                "home.html", error="Room does not exsit", code=code, name=name
            )
